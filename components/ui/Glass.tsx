...
export const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label?: string }> = ({ label, className = '', children, ...props }) => (
  <div className="w-full">
    {label && <label className="block text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wider">{label}</label>}
    <select
      className={`w-full bg-white/50 border border-gray-200/60 rounded-xl px-4 py-2.5 text-orqio-black focus:outline-none focus:ring-2 focus:ring-orqio-orange/50 focus:bg-white transition-all appearance-none cursor-pointer ${className}`}
      {...props}
    >
      {children}
    </select>
